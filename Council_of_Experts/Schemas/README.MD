...
**Deliberation Layer (optional):** The Council supports an optional Deliberation stage controlled by `UNICC_DELIBERATION_ACTIVE`. When enabled, each expert critiques the other two experts' outputs (critique round) and then responds to critiques received (defense round) before final arbitration. Experts may update their recommended action and overall status based on peer critiques. When `UNICC_DELIBERATION_ACTIVE=false`, experts operate fully independently and arbitration runs directly on initial outputs.
